---
type: meta
title: 知识库索引
created: 2026-04-10
updated: 2026-07-30
---

# 📚 Wiki 索引

## 来源摘要 (Sources)

| 页面 | 来源 | 数据完整度 |
|------|------|-----------|
| [[数学觉醒]] | 《数学觉醒：学会更清晰地思考》 | 第1-2章完整摘要，3-20章标题推断（原文受限） |
| [[pbrt]] | 《Physically Based Rendering》4th Ed. | ✅ 全书完整（16章+3附录+前言，167文件，全部双语翻译） |
| [[llm-wiki]] | LLM Wiki 方法论（Andrej Karpathy） | ✅ 完整（原文全文） |
| [[fcg]] | 《Fundamentals of Computer Graphics》4th Ed.（虎书） | ✅ 全书提取（26章+前言，27文件，待双语翻译） |
| [[AI对话管理黑板理论]] | AI 对话管理：Context Window 作为有限黑板 | ✅ 完整摄入 |
| [[三角洲战术设计与双端体验]] | 三角洲行动战术设计与双端体验 | 已建来源页 |
| [[mcp-vs-cli]] | 为什么越来越多的人抛弃 MCP，转向 CLI？ | ✅ 完整（视频文稿全文） |

## 实体 (Entities)

| 页面 | 类型 | 关联来源 |
|------|------|---------|
| [[戴维·贝西]] | 人物 | 《数学觉醒》作者 |
| [[Steve Marschner]] | 人物 | 虎书第一作者 |
| [[Peter Shirley]] | 人物 | 虎书作者 |
| [[Matt Pharr]] | 人物 | pbrt 第一作者 |
| [[Wenzel Jakob]] | 人物 | pbrt 作者 |
| [[Greg Humphreys]] | 人物 | pbrt 作者 |
| [[Pat Hanrahan]] | 人物 | pbrt 核心影响者 |
| [[Andrej Karpathy]] | 人物 | LLM Wiki 方法论作者 |
| [[马克的技术工作坊]] | 频道/人物 | [[mcp-vs-cli]] 作者 |

## 概念 (Concepts)

### 来自《数学觉醒》

| 页面 | 简述 |
|------|------|
| [[数学直觉]] | 数学思维的核心能力 |
| [[内化学习]] | 像学用勺子一样内化技能 |
| [[具身认知]] | 思维与身体经验的联系 |
| [[数学焦虑]] | 恐惧如何阻塞思考 |
| [[刻意练习]] | 没有窍门，只有持续训练 |
| [[视觉化思维]] | 用图形和意象理解抽象 |

### 来自 pbrt

| 页面 | 简述 |
|------|------|
| [[光线追踪]] | pbrt 的核心算法，从 Whitted 1980 到现代 |
| [[蒙特卡洛积分]] | 高维渲染积分的数学基础 |
| [[文学编程]] | Knuth 提出的编程方法论，pbrt 的写作范式 |
| [[光谱渲染]] | 第4版核心特性——弃用 RGB，使用波长采样 |
| [[GPU渲染]] | 波前架构，10-100× 性能提升 |
| [[路径追踪]] | 全局光照的行业标准算法 |
| [[BSDF]] | 双向散射分布函数——描述表面光散射 |
| [[辐射度量学]] | 精确描述光传输的物理学科 |
| [[BVH加速结构]] | 光线求交的层次化加速数据结构 |
| [[仿射变换]] | 齐次坐标与 4×4 矩阵统一表示平移/旋转/缩放 |
| [[球面几何]] | 立体角、球坐标、球面三角——方向空间的数学基础 |
| [[采样与重建]] | 傅里叶分析、走样/反走样、低偏差序列与 QMC |
| [[形状系统]] | Shape/Primitive 两层抽象、七种几何图元 |
| [[反射模型]] | 从漫反射到微表面的完整反射模型体系 |
| [[纹理与材质]] | 纹理参数化 BSDF、MIP 映射反走样 |
| [[体积散射]] | 参与介质中的光散射 |
| [[光源]] | 光源类型与多光源采样策略 |
| [[相机与胶片]] | 相机模型、胶片与传感器响应 |
| [[光传输方程]] | 渲染方程的数学形式与积分器 |

### 来自 LLM Wiki

| 页面 | 简述 |
|------|------|
| [[RAG与Wiki范式]] | 两种 LLM 知识管理范式的根本对比 |
| [[知识复利]] | Wiki 随每次 ingest 和 query 不断增值的机制 |
| [[人机协作知识管理]] | 人类策展 + LLM 维护的分工模式 |

### 来自 AI Agent

| 页面 | 简述 |
|------|------|
| [[上下文管理]] | 有限 Context Window 中的组织、剪枝与固化 |
| [[MCP]] | 结构化工具协议：可控安全，但工具元信息吃 Token |
| [[CLI作为Agent工具接口]] | bash + 可组合 CLI：省 Token、链路短，灵活但风险更大 |
| [[Agent Skill]] | 按需说明书；补齐冷门 CLI / 工作流约定 |

## 分析 (Analyses)

| 页面 | 简述 |
|------|------|
| [[pbrt两个月阅读计划]] | 基于 [[pbrt]] 与 [[fcg]] 的 8 周首遍阅读拆解：周计划、补位章节、最低完成标准与掉队补救策略 |
| [[pbrt Day 3 详细计划（1.3 pbrt System Overview）]] | 第 1 周第 3 天阅读计划：建立系统地图，抓住 14 个基类型、三阶段执行与 `Integrator → Li()` 主线 |
| [[pbrt Day 4 详细计划（1.4 How to Proceed through This Book + 1.5 Using and Understanding the Code）]] | 第 1 周第 4 天阅读计划：建立首遍阅读守则、源码导航图与“先别深挖”清单 |
| [[pbrt Day 6 详细计划（2 Monte Carlo Integration + 2.1 Monte Carlo Basics）]] | 第 1 周第 6 天阅读计划：跨进 Monte Carlo 主线，只抓“为什么渲染要靠 MC”与 `期望 / 无偏 / 方差 / 收敛率` 四个核心词 |
| [[pbrt Day 7 详细计划（2.2 Improving Efficiency + 2.3 / 2.4 选读）]] | 第 1 周第 7 天阅读计划：从“MC 为什么成立”转向“MC 如何更省样本、更少噪声”，抓住分层采样、重要性采样、MIS 与效率技巧 |
| [[pbrt Day 8 详细计划（3 Geometry and Transformations 前半）]] | 第 2 周第 1 天阅读计划：进入几何基础设施主线，先建立坐标系、点/向量/法线语义与 Ray 的统一心智模型 |

---

*最后更新：2026-07-30（ingest [[mcp-vs-cli]]：MCP / CLI / Agent Skill）*
